...
%% Bitrate
% fig = openfig('report/figures/qu8DiloHtmaTe10Daur.fig');
% figure(fig);
% ax = gca;
...
%% KNN accuracy
% fig = openfig('report/figures/clknnQu8Daur.fig');
% figure(fig);
% ax = gca;
% ax.XTickLabels = real_fps;
% xlabel('Frames per second'), ylabel('KNN-5 accuracy'), grid on;
% load('report/tables/fp0Me40Qu0DaurClknn.mat');
...
%% CSII
fig = openfig('report/figures/qu8Dasp.fig');
figure(fig);
xlabel('Mel bands'), ylabel('CSII indicator'), grid on;
load('report/tables/fp0Me40Qu0Dasp.mat');
