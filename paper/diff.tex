...
\cite{can2015, brocolini2013, nilsson2007}}%DIFAUXCMD
.
}
...
%DIFDELCMD < %%%
\DIFdelend A more detailed review of used methods is available in \cite{chachada2013}.
...
\subsection{Event recognition} \DIFaddbegin \label{sec:event_p}
\DIFaddend 
...
\citep{ntalampiras2013}}%DIFAUXCMD
, deep learning approaches \mbox{%DIFAUXCMD
\citep{salamon2017} }%DIFAUXCMD
...
\DIFaddbegin \begin{figure*}[htbp]
	...
	\caption{\DIFaddFL{Classification process flowchart. Here third-octave bands are obtained by reversing the operations in Section~\ref{sec:dataenc}. Cepstral coefficients are then computed before training the model. Cross-validation is achieved by independantly training and testing the models with each of the 10 combination of a 90\%-10\% training-validation dataset separation. Accuracy results are considered only for validation (not encountered during training) data.}}
...
\citep{ntalampiras2008}}%DIFAUXCMD
