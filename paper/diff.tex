...
%DIF DEL article_rev.tex   Fri Nov 24 12:45:44 2017
%DIF ADD articleV3.tex     Mon Nov 27 11:52:41 2017
...
\copyrightyear{2017}
%DIF 56-57c56-57
%DIF < \externaleditor{Academic Editor: name}
%DIF < \history{Received: date; Accepted: date; Published: date}
%DIF -------
%\externaleditor{Academic Editor: name} %DIF > 
\history{Received: 17 October 2017; Accepted: 20 November 2017; Published: date} %DIF > 
%DIF -------
...
%DIF 76c76
%DIF < \Author{\hl{F\'elix Gontier} $^{1}$, Mathieu Lagrange $^{1,}$\hl{*}, Pierre Aumond $^{2,3}$, Arnaud Can $^{2}$ and Catherine Lavandier $^{3}$}%Please carefully check the accuracy of names and affiliations. Changes will not be possible after proofreading.   and  ``*'' is newly added, please confirm.
%DIF -------
\Author{{F\'elix Gontier} $^{1}$, Mathieu Lagrange $^{1,}${*}, Pierre Aumond $^{2,3}$, Arnaud Can $^{2}$ and Catherine Lavandier $^{3}$}%Please carefully check the accuracy of names and affiliations. Changes will not be possible after proofreading.   and  ``*'' is newly added, please confirm. %R:confirmed that M.L. is the corresponding author %DIF > 
%DIF -------
...
\address{%
%DIF 84-87c84-87
%DIF < $^{1}$ \quad \hl{LS2N, Nantes Cedex}, France; felix.gontier@ls2n.fr\\%please add the complete location and add it's zip code.
%DIF < $^{2}$ \quad \hl{IFSTTAR, AME, LAE}; pierre.aumond@ifsttar.fr (P.A.); arnaud.can@ifsttar.fr (A.C.)\\%please add the complete location and add it's zip code.
%DIF < $^{3}$ \quad \hl{ETIS, UMR 8051}%please add the complete location and add it's zip code. 
%DIF < , Universit\'e Paris Seine, Universit\'e Cergy-Pontoise, ENSEA, CNRS, F-95000 \hl{Cergy-Pontoise}; %please add the city.
%DIF -------
$^{1}$ \quad {LS2N}{, UMR 6004, CNRS, \'Ecole Centrale de Nantes, 44300 Nantes}, France; felix.gontier@ls2n.fr\\%please add the complete location and add its zip code. %R: done %DIF > 
$^{2}$ \quad {IFSTTAR, AME, LAE}{, 44340 Bouguenais, France}; pierre.aumond@ifsttar.fr (P.A.); arnaud.can@ifsttar.fr (A.C.)\\%please add the complete location and add it's zip code. %R: done %DIF > 
$^{3}$ \quad {ETIS, UMR 8051}%please add the complete location and add it's zip code.  %DIF > 
, Universit\'e Paris Seine, Universit\'e de Cergy-Pontoise, ENSEA, CNRS, F-95000 {Cergy-Pontoise}; %please add the city. %R: done %DIF > 
%DIF -------
...
The advent of low cost acoustic sensors together with the need to better monitor and comprehend the acoustic environment of urban and wilderness areas give rise to the deployment of experimental sensor networks such as the \DIFdelbegin %DIFDELCMD < \hl{SONYC} %%%
\DIFdelend \DIFaddbegin {\DIFadd{SONYC}} \DIFaddend ({\url{htttp://wp.nyu.edu/sonyc}}) \cite{mydlarz2017implementation} and %DIF < Footnote are not allowed, pls conrfim.
 \DIFdelbegin %DIFDELCMD < \hl{CENSE} %%%
\DIFdelend %DIF > Footnote are not allowed, pls conrfim. %R: ok
 \DIFaddbegin {\DIFadd{CENSE}} \DIFaddend ({\url{htttp://cense.ifsttar.fr}}) \cite{picault2017} projects. To do so, considering the so-called ``sensor grid'' approach  \cite{lim2005sensor,tham2005sensorgrid} has several advantages. The sensor grid approach puts the focus \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin {\DIFadd{on:}} \DIFaddend (a) a distributed system of data acquisition with a large set of sensors; (b) the production and storage of a large dataset; and (c) its availability for intensive and open data analysis computation.

In such approach, the number of sensor nodes shall be extended as needed without having to change the hardware and software architecture, i.e., the approach shall be scalable. Second, the nodes shall be energy efficient, \DIFdelbegin \DIFdel{ideally autonomous}\DIFdelend \DIFaddbegin {\DIFadd{and,  ideally}\DIFaddend ,\DIFdelbegin \DIFdel{in order to}\DIFdelend \DIFaddbegin } \DIFadd{autonomous, }{\DIFadd{to}} \DIFaddend ease the deployment of the sensor grid to the desired topology. Third, the encoding scheme used to transmit the data from the sensor to the storage and processing servers shall be designed with care in order to guarantee the privacy of the citizens. At the same time, the data stored shall remain as rich as \DIFdelbegin \DIFdel{possible }\DIFdelend \DIFaddbegin {\DIFadd{possible,}} \DIFaddend while maintaining a low bitrate for efficient transmission and storage.

In the case of urban acoustic monitoring, the data of interest typically \DIFdelbegin \DIFdel{consists }\DIFdelend \DIFaddbegin {\DIFadd{consist}} \DIFaddend of standardized high-quality measurements of energetic indicators such as the equivalent sound level $L_{eq}$ in dB SPL or its A-weighted counterpart $L_{Aeq}$ in dBA. However, while these indicators have proven to correlate well with perceptual evaluations for negative impact sound environments \cite{gozalo2015}, they are not sufficient to fully describe urban sound environments \cite{rychtarikova2013}. Many other variables can be derived to better account for previously implicit properties \cite{can2016} including percentile values or time dynamics. Studies have been conducted to select relevant subsets of descriptors in sound environment characterization \cite{can2015, brocolini2013, nilsson2007}.

In addition, considering sound sources allows \DIFdelbegin \DIFdel{to better assess }\DIFdelend \DIFaddbegin {\DIFadd{better assessing}} \DIFaddend the human perception of the urban sound environment in terms of pleasantness and other perceptual indicators \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{lavandier2006contribution, aumond2017modeling}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{lavandier2006contribution, aumond2017}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . For instance, \DIFdelbegin \DIFdel{a bird song}\DIFdelend \DIFaddbegin {\DIFadd{bird song}} \DIFaddend and traffic noise with the same energy are perceived very differently. Most acoustic indicators used in current sound maps describe the global sound environment and lack information on its composition. The recognition and the estimation of the level of contribution of sources of interest is thus of great importance to fully model the studied acoustic environments\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin { \DIFadd{(}\DIFaddend see for example \cite{alsina2016design, app7020146, gloaguen2016estimating}\DIFaddbegin \DIFadd{)}}\DIFaddend . Other indicators have been found to correlate with specific sound environment content \cite{aumond2017} but do not achieve the level of performances of a well-designed source detection scheme in characterizing acoustic environments. This source detection step can\DIFdelbegin \DIFdel{respectively be}\DIFdelend \DIFaddbegin { \DIFadd{be}} \DIFaddend operated online (on the sensors) or offline \DIFaddbegin {\DIFaddend (on the data servers)\DIFaddbegin \DIFadd{,}} \DIFaddend leading to different grid designs (Figure \ref{fig:codingScheme}).
...
\DIFdelbegin \DIFdel{In order to }\DIFdelend \DIFaddbegin {\DIFadd{To}} \DIFaddend promote reproducible research, the coder as well as the experiments needed to generate the figures is available to the research \DIFdelbegin %DIFDELCMD < \hl{community} %%%
\DIFdelend \DIFaddbegin {\DIFadd{community}} \DIFaddend ({\url{https://github.com/felixgontier/cense-coder}}). The remaining of the paper is organized as follows: Section \ref{sec:coder} describes the proposed encoding \DIFdelbegin \DIFdel{scheme. The }\DIFdelend \DIFaddbegin {\DIFadd{scheme;}} \DIFadd{the }\DIFaddend experimental protocol designed to validate the proposed approach is described in \DIFdelbegin \DIFdel{Section \ref{sec:protocol} }\DIFdelend \DIFaddbegin {\DIFadd{Section \ref{sec:protocol};}} \DIFaddend and results are discussed in Section \ref{sec:results}.

\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{figures/censeCoder}
\caption{Three alternative implementations of the sensor grid approach for the monitoring of the acoustic \DIFaddbeginFL {\DIFaddendFL environment\DIFdelbeginFL \DIFdelFL{. In }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{:}} \DIFaddFL{in }\DIFaddendFL (\textbf{A}) the raw audio is transmitted; in (\textbf{B}) only the detected events; and in (\textbf{C}) a compressed spectral representation. Thickness of arrows indicates bandwidth and thickness of boxes indicates the level of computation or storage required.}
...
%DIF > NOTE: Please add author names so that references are not subjects: "Smith et al. [ref] add..."
...
A common way to implement third-octave analysis is to first design the highest octave three filters, and use them on progressively time-decimated versions of the input signal \cite{davis1986}. \DIFdelbegin %DIFDELCMD < \hl{Ref.} %%%
\DIFdelend \DIFaddbegin {\DIFadd{Antoni}} \DIFaddend %newly added information, please confirm.
...
The aim of this experiment is two-fold: to evaluate the global loss of information induced by the coding process, and to evaluate the adequacy of third-octave bands indicators for classification. As~studying the best performing classical \DIFdelbegin %DIFDELCMD < \hl{models} %%%
\DIFdelend \DIFaddbegin {\DIFadd{models}} \DIFaddend \citep{ntalampiras2013}, %Reference [49] is cited after [47] directly, which is out of order. Please confirm.
...
\begin{figure}[H]
	...
	\caption{Classification process flowchart. Here third-octave bands are obtained by reversing the operations in Section~\ref{sec:dataenc} and cepstral coefficients are computed before training the model. Cross-validation is achieved by independantly training and testing the models with each of the 10 \DIFdelbeginFL \DIFdelFL{combination }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{combinations }\DIFaddendFL of a 90--\DIFdelbeginFL %DIFDELCMD < \hl{100}%%%
\DIFdelendFL \DIFaddbeginFL {\DIFaddFL{10}}\DIFaddendFL \% \DIFdelbeginFL \DIFdelFL{training-validation }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{training}{\DIFaddFL{--}}\DIFaddFL{validation }\DIFaddendFL dataset separation.}% changed from ``10'' to ``100'', please confirm.
%DIF > NOTE: 90%/10%?
%DIF > R: 90% of the dataset used for training, the remaining 10% for validation
...
It is important that the proposed scheme ensures a high level of recognition of acoustic events but, as the data \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin {\DIFadd{are}} \DIFaddend transmitted over the network and potentially stored, the level of intelligibility of  speech utterances potentially captured in the decoded stream should be as low as possible. Thus, intelligibility in decoded and reconstructed audio is also an important concern of this study.

The use of an audio recording as an evidence is considered by the forensic phonetics \cite{baldwin1990forensic} field. The readers shall be aware that even recorded with a good quality, such recording remains a weak biometrical indicator \cite{boe2000forensic}. \DIFdelbegin \DIFdel{Still}\DIFdelend \DIFaddbegin {\DIFadd{However}}\DIFaddend , even if the identity of the speaker cannot be asserted, one can attempt to transcribe the spoken utterances. \DIFdelbegin \DIFdel{As such}\DIFdelend \DIFaddbegin {\DIFadd{Thus}}\DIFaddend , we believe that it is important to demonstrate that the proposed encoding scheme do not allow such thing.

\DIFdelbegin \DIFdel{In order to }\DIFdelend \DIFaddbegin {\DIFadd{To}} \DIFaddend decrease the intelligibility, several approaches can be considered. First, a speech versus non speech detector can be implemented on the sensor. If some speech is detected, the data \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin {\DIFadd{are}} \DIFaddend not transmitted. This approach is however prone to failure of the detector and will lead to unavailability of data during speech time \DIFdelbegin \DIFdel{periods }\DIFdelend \DIFaddbegin {\DIFadd{periods,}} \DIFaddend which might compromise the relevance of some indicators.
...
Thus, the impact of the frame rate parameter of the proposed encoding scheme on intelligibility is evaluated. To do so, a perceptual test is conducted. For the evaluation, a dataset of clean speech utterances recorded in studio conditions is used. The dataset is composed of \DIFdelbegin \DIFdel{9 }\DIFdelend \DIFaddbegin {\DIFadd{nine}} \DIFaddend french sentences enunciated by \DIFdelbegin \DIFdel{3 male and 3 }\DIFdelend \DIFaddbegin {\DIFadd{three}} \DIFadd{male and }{\DIFadd{three}} \DIFaddend female speakers. The sentences have a mean duration of $2$~s and are composed of \DIFdelbegin \DIFdel{5 to 12 }\DIFdelend \DIFaddbegin {\DIFadd{5--12}} \DIFaddend words, averaging \DIFdelbegin \DIFdel{at 8. }\DIFdelend \DIFaddbegin {\DIFadd{eight words}}\DIFadd{. }\DIFaddend The 54 extracts are encoded using the proposed approach with \DIFdelbegin \DIFdel{4 }\DIFdelend \DIFaddbegin {\DIFadd{four}} \DIFaddend analysis frame rate parameters \DIFdelbegin \DIFdel{of respectively }\DIFdelend \DIFaddbegin {\DIFadd{of,  respectively,}} \DIFaddend 50~Hz, 16~Hz, 8~Hz and 4~Hz leading to 216 audio excerpts being tested in total. However, as the recording conditions are the same for all extracts the speaker identity is considered as a source of variability rather than as a necessary factor in the test statistics. As a \DIFdelbegin \DIFdel{result }\DIFdelend \DIFaddbegin {\DIFadd{result,}} \DIFaddend the objective is primarily to evaluate the \DIFdelbegin \DIFdel{9 sentences with 4 }\DIFdelend \DIFaddbegin {\DIFadd{nine}} \DIFadd{sentences with }{\DIFadd{four}} \DIFaddend encoding frame rates, i.e., 36 parameters.
...
%DIF > NOTE: Or "Griffin--Lim"? %R: Changed to "Griffin--Lim"
...
\DIFaddbegin \begin{figure}[H]
    ...
    \begin{subfigure}[H]{0.47\textwidth}
       ...
       \includegraphics[width=1\textwidth]{figures/err_m_u_f_r.eps}
        \caption{\DIFaddFL{Fast analysis, rectangular window, no overlap}}
    \end{subfigure}
    \begin{subfigure}[H]{0.47\textwidth}
        \centering
        \includegraphics[width=1\textwidth]{figures/err_m_u_f_h.eps}
        \caption{\DIFaddFL{Fast analysis, Hann window, 66\% overlap}}
    \end{subfigure}
	\begin{subfigure}[H]{0.47\textwidth}
        \centering
        \includegraphics[width=1\textwidth]{figures/err_m_u_s_r.eps}
        \caption{\DIFaddFL{Slow analysis, rectangular window, 50\% overlap}}
    \end{subfigure}
    \begin{subfigure}[H]{0.475\textwidth}
        \centering
        \includegraphics[width=1\textwidth]{figures/err_m_u_s_h.eps}
        \caption{\DIFaddFL{Slow analysis, Hann window, 66\% overlap}}
    \end{subfigure}
    \vspace{-10pt}
    \caption{\DIFaddFL{Measurement error of third-octave bands over two-seconds environmental sounds recordings (US8k dataset). For most }{\DIFaddFL{settings,}} \DIFaddFL{the FFT-based method yields similar errors compared to the time-domain filtering reference. }\label{fig:errormu}}
\end{figure}
...
\begin{figure}[H]
	\centering
		\DIFdelbeginFL %DIFDELCMD < \begin{subfigure}[H]{0.47\textwidth}
%DIFDELCMD <         \centering
%DIFDELCMD <        \includegraphics[width=1\textwidth]{figures/err_m_u_f_r.eps}
%DIFDELCMD <         %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
...
%DIFDELCMD <     \begin{subfigure}[H]{0.47\textwidth}
%DIFDELCMD <         \centering
%DIFDELCMD <         \includegraphics[width=1\textwidth]{figures/err_m_u_f_h.eps}
%DIFDELCMD <         %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
...
%DIFDELCMD < 	\begin{subfigure}[H]{0.47\textwidth}
%DIFDELCMD <         \centering
%DIFDELCMD <         \includegraphics[width=1\textwidth]{figures/err_m_u_s_r.eps}
%DIFDELCMD <         %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
...
%DIFDELCMD <     \begin{subfigure}[H]{0.47\textwidth}
%DIFDELCMD <         \centering
%DIFDELCMD <         \includegraphics[width=1\textwidth]{figures/err_m_u_s_h.eps}
%DIFDELCMD <         %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Measurement error of third-octave bands over two-seconds environmental sounds recordings (US8k dataset). For most settings the FFT-based method yields similar errors compared to the time-domain filtering reference. }%DIFDELCMD < \label{fig:errormu}%%%
}
%DIFAUXCMD
...
The same set of parameters as in bitrate evaluation is used to study the impact of their tuning on sound event recognition performance. This experiment aims at finding ways to further reduce the encoded data size without strongly affecting detection performance. Results of the four presented classification methods \DIFdelbegin %DIFDELCMD < \hl{Section} %%%
\DIFdel{\ref{sec:event_p}}\DIFdelend \DIFaddbegin \DIFadd{(}{\DIFadd{Section}} \DIFadd{\ref{sec:event_p}) }\DIFaddend are provided for the sake of completeness.%DIF <  newly added information, please confirm.
%DIF >  newly added information, please confirm. R:correct
...
%DIFDELCMD < \begin{figure}[H]
%DIFDELCMD < 	\centering
%DIFDELCMD < 		\includegraphics[width=0.65\textwidth]{figures/class_mel_q.eps}
%DIFDELCMD < 	%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Classification accuracy as a function of word size before encoding. The baseline plotted on the right is computed without quantizing the representation.}}
	%DIFAUXCMD
%DIFDELCMD < \label{fig:class_mel_q}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 
...
\begin{itemize}[leftmargin=*,labelsep=5.5mm]
	\item \DIFdelbegin \DIFdel{precision: the }\DIFdelend \DIFaddbegin \DIFadd{Precision: }{\DIFadd{The}} \DIFaddend impact of the quantization step for 8 bits codewords is found to be very small \DIFaddbegin {\DIFaddend (<~0.2 dB)\DIFaddbegin \DIFadd{,}} \DIFaddend and thus:
	\begin{itemize}[leftmargin=*,labelsep=5.6mm]
	\item does not impact the quality of the acoustics indicators (Sections~\ref{sec:efficiency_p}  \DIFaddbegin { \DIFaddend and \ref{sec:efficiency_r})\DIFdelbegin \DIFdel{.
	}\DIFdelend \DIFaddbegin \DIFadd{; and}}
	\DIFaddend \item enables \DIFdelbegin \DIFdel{the sound }\DIFdelend \DIFaddbegin \DIFadd{sound event }\DIFaddend recognition with the same quality than directly from the spectrogram (Sections~\ref{sec:event_p}, \ref{sec:event_r} and \ref{sec:tob_r}).
	\end{itemize}
	\item \DIFdelbegin \DIFdel{efficiency: using }\DIFdelend \DIFaddbegin \DIFadd{Efficiency: }{\DIFadd{Using}} \DIFaddend the high precision parameters, the bitrate is about 1.4~kbps and 0.4~kbps for fast and slow modes respectively (Section~\ref{sec:efficiency_r}).
	\item \DIFdelbegin \DIFdel{privacy: at }\DIFdelend \DIFaddbegin \DIFadd{Privacy: }{\DIFadd{At}} \DIFaddend all operating modes, the reverted audio signal is found to be inintelligible according to a perceputal test done using clean speech recorded at very low background level. Additionaly, the sensors being typically hanged at 4 m high in a usually very noisy environment, we believe that the intelligibility risk of the proposed approach is negligible (Sections~\ref{sec:inintelligibility_p} and \ref{sec:inintelligibility_r}).
\end{itemize}
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend %DIF > NOTE: All capitalization
...
\DIFdelbegin %DIFDELCMD < \authorcontributions{\hl{For research articles with several authors, }a short paragraph specifying their individual contributions must be provided. The following statements should be used “X.X. and Y.Y. conceived and designed the experiments; X.X. performed the experiments; X.X. and Y.Y. analyzed the data; W.W. contributed reagents/materials/analysis tools; Y.Y. wrote the paper.” Authorship must be limited to those who have contributed substantially to the work reported.}%%%
%DIF < Please add the main text in accordance with the template.
\DIFdelend %DIF > \authorcontributions{{For research articles with several authors, }a short paragraph specifying their individual contributions must be provided. The following statements should be used “X.X. and Y.Y. conceived and designed the experiments; X.X. performed the experiments; X.X. and Y.Y. analyzed the data; W.W. contributed reagents/materials/analysis tools; Y.Y. wrote the paper.” Authorship must be limited to those who have contributed substantially to the work reported.}%Please add the main text in accordance with the template.
\DIFaddbegin \authorcontributions{F. Gontier and M. Lagrange conceived and designed the experiments; F. Gontier performed the experiments; F. Gontier, M. Lagrange, P. Aumond, A. Can and C. Lavandier analyzed the data; P. Aumond contributed analysis tools; F. Gontier, M. Lagrange, P. Aumond, A. Can and C. Lavandier wrote the paper.}
\DIFaddend 

\DIFdelbegin %DIFDELCMD < \conflictsofinterest{\hl{The authors declare no conflict of interest}.}%%%
%DIF < Please disclose conflicts of interest, or add 'The authors declare no conflicts of interest'.
\DIFdelend \DIFaddbegin \conflictsofinterest{The authors declare no conflict of interest.}%DIF > Please disclose conflicts of interest, or add 'The authors declare no conflicts of interest'. %R: No conflicts of interest
\DIFaddend %=====================================
% References, variant B: external bibliography
%=====================================
\DIFaddbegin \externalbibliography{yes}
%DIF > \bibliography{your_external_BibTeX_file}
\DIFaddend \reftitle{References}
\begin{thebibliography}{999}
\DIFaddbegin \providecommand{\natexlab}[1]{#1}
\DIFaddend 

\DIFdelbegin %DIFDELCMD < \bibitem[Mydlarz \em{et~al.}(2017)Mydlarz, Salamon, and
%DIFDELCMD <   Bello]{mydlarz2017implementation}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibitem[Mydlarz et~al.(2017)Mydlarz, Salamon, and
  Bello]{mydlarz2017implementation}
\DIFaddend Mydlarz, C.; Salamon, J.; Bello, J.
\newblock The implementation of low-cost urban acoustic monitoring devices.
\newblock {\em \DIFdelbegin \DIFdel{Applied Acoustics}\DIFdelend \DIFaddbegin \DIFadd{Appl.~Acoust.}\DIFaddend } {\bf 2017}, {\em 117},~207--218.

\DIFdelbegin %DIFDELCMD < \bibitem[Picaut \em{et~al.}(2017)Picaut, Can, Ardouin, Cr{\'e}peaux, Dhorne,
%DIFDELCMD <   {\'E}coti{\`e}re, and Lagrange]{picault2017}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibitem[Picaut et~al.(2017)Picaut, Can, Ardouin, Cr{\'e}peaux, Dhorne,
  {\'E}coti{\`e}re, and Lagrange]{picault2017}
\DIFaddend Picaut, J.; Can, A.; Ardouin, J.; Cr{\'e}peaux, P.; Dhorne, T.;
...
\newblock   \DIFdelbegin \DIFdel{The Journal of the Acoustical Society of America. Boston, 2017.
}\DIFdelend \DIFaddbegin \emph{\DIFadd{J. Acoust. Soc. Am. }}  \textbf{\DIFadd{2017}}\DIFadd{, }\emph{\DIFadd{141}}\DIFadd{, 3808.
}\DIFaddend 

\DIFdelbegin %DIFDELCMD < \bibitem[Lim \em{et~al.}(2005)Lim, Teo, Mukherjee, Lam, Wong, and
%DIFDELCMD <   See]{lim2005sensor}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibitem[Lim et~al.(2005)Lim, Teo, Mukherjee, Lam, Wong, and
  See]{lim2005sensor}
\DIFaddend Lim, H.; Teo, Y.; Mukherjee, P.; Lam, V.; Wong, W.; See, S.
\newblock Sensor grid: \DIFdelbegin \DIFdel{integration }\DIFdelend \DIFaddbegin \DIFadd{Integration }\DIFaddend of wireless sensor networks and the grid.
\newblock \DIFdelbegin \DIFdel{Local Computer Networks, 2005. }\DIFdelend \DIFaddbegin \DIFadd{In Proceedings of the 2005 }\DIFaddend 30th Anniversary \DIFdelbegin \DIFdel{. The }\DIFdelend IEEE Conference on \DIFdelbegin \DIFdel{. IEEE, 2005,  }\DIFdelend \DIFaddbegin \DIFadd{Local Computer Networks, }{\DIFadd{Sydney,  Australia, 17 November }} \DIFadd{2005; }\DIFaddend pp. 91--99.
 \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend %DIF > please confirm if the added information is right. %R: Confirmed
\bibitem[Tham and Buyya(2005)]{tham2005sensorgrid}
Tham, C.K.; Buyya, R.
\newblock SensorGrid: Integrating sensor networks and grid computing.
\newblock {\em CSI \DIFdelbegin \DIFdel{communications}\DIFdelend \DIFaddbegin \DIFadd{Commun.}\DIFaddend } {\bf 2005}, {\em 29},~24--29.

\DIFdelbegin %DIFDELCMD < \bibitem[Rey~Gozalo \em{et~al.}(2015)Rey~Gozalo, Trujillo~Carmona,
%DIFDELCMD <   Barrigon~Morillas, and Gomez~Escobar]{gozalo2015}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibitem[Rey~Gozalo et~al.(2015)Rey~Gozalo, Trujillo~Carmona,
  Barrigon~Morillas, and Gomez~Escobar]{gozalo2015}
\DIFaddend Rey~Gozalo, G.; Trujillo~Carmona, J.; Barrigon~Morillas, J.; Gomez~Escobar, V.
\newblock Relationship between objective acoustic indices and subjective
  assessments for the quality of soundscapes.
\newblock {\em \DIFdelbegin \DIFdel{Applied Acoustics}\DIFdelend \DIFaddbegin \DIFadd{Appl. Acoust.}\DIFaddend } {\bf 2015}, {\em 97},~1--10.
...
\newblock Soundscape categorization on the basis of objective acoustical
  parameters.
\newblock {\em \DIFdelbegin \DIFdel{Applied Acoustics}\DIFdelend \DIFaddbegin \DIFadd{Appl. Acoust.}\DIFaddend } {\bf 2013}, {\em 74},~240--247.

\DIFdelbegin %DIFDELCMD < \bibitem[Can \em{et~al.}(2016)Can, Aumond, Michel, De~Coensel, Ribeiro,
%DIFDELCMD <   Botteldooren, and Lavandier]{can2016}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibitem[Can et~al.(2016)Can, Aumond, Michel, De~Coensel, Ribeiro,
  Botteldooren, and Lavandier]{can2016}
\DIFaddend Can, A.; Aumond, P.; Michel, S.; De~Coensel, B.; Ribeiro, C.; Botteldooren, D.;
  Lavandier, C.
\newblock Comparison of noise indicators in an urban context.
\newblock  \DIFaddbegin \DIFadd{In Proceedings of the }\DIFaddend 45th International Congress and Exposition on Noise Control
  Engineering, \DIFdelbegin \DIFdel{2016, }\DIFdelend \DIFaddbegin {\DIFadd{Hamburg, Germany, 21--24 August }}\DIFadd{2016; }\DIFaddend pp. 5678--5686.
 \DIFdelbegin %DIFDELCMD < 
...
\newblock {\em J. \DIFdelbegin \DIFdel{Ac}\DIFdelend \DIFaddbegin \DIFadd{Acoust}\DIFaddend . Soc. Am.} {\bf 2015}, {\em 137},~208--218.

\DIFdelbegin %DIFDELCMD < \bibitem[Brocolini \em{et~al.}(2013)Brocolini, Lavandier, Quoy, and
%DIFDELCMD <   Ribeiro]{brocolini2013}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibitem[Brocolini et~al.(2013)Brocolini, Lavandier, Quoy, and
  Ribeiro]{brocolini2013}
\DIFaddend Brocolini, L.; Lavandier, C.; Quoy, M.; Ribeiro, C.
\newblock Measurements of acoustic environments for urban soundscapes: Choice
  ...
  indicators.
\newblock {\em J.\DIFdelbegin \DIFdel{Ac}\DIFdelend \DIFaddbegin \DIFadd{~Acoust}\DIFaddend . Soc. Am.} {\bf 2013}, {\em 134},~813--821.

\DIFdelbegin %DIFDELCMD < \bibitem[Nilsson \em{et~al.}(2007)Nilsson, Botteldooren, and
%DIFDELCMD <   De~Coensel]{nilsson2007}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibitem[Nilsson et~al.(2007)Nilsson, Botteldooren, and
  De~Coensel]{nilsson2007}
\DIFaddend Nilsson, M.; Botteldooren, D.; De~Coensel, B.
\newblock Acoustic indicators of soundscape quality and noise annoyance in
  outdoor urban areas.
\newblock  \DIFaddbegin \DIFadd{In Proceedings of the }\DIFaddend 19th International Congress on Acoustics, \DIFaddbegin {\DIFadd{Madrid, Spain, 2--7 September}} \DIFaddend 2007.
\DIFdelbegin %DIFDELCMD < 
...
\newblock The contribution of sound source characteristics in the assessment of
  urban soundscapes.
\newblock {\em Acta \DIFdelbegin \DIFdel{Acustica united with Acustica}\DIFdelend \DIFaddbegin \DIFadd{Acust. United Acust.}\DIFaddend } {\bf 2006}, {\em
  92},~912--921.

\DIFdelbegin %DIFDELCMD < \bibitem[Aumond \em{et~al.}(2017)Aumond, Can, De~Coensel, Botteldooren,
%DIFDELCMD <   Ribeiro, and Lavandier]{aumond2017modeling}
%DIFDELCMD < %%%
\DIFdel{Aumond, P.; Can, A.; De~Coensel, B.; Botteldooren, D.; Ribeiro, C.; Lavandier,
  C.
}%DIFDELCMD < \newblock %%%
\DIFdel{Modeling Soundscape Pleasantness Using perceptual Assessments and
  Acoustic Measurements Along Paths in Urban Context.
}%DIFDELCMD < \newblock {\em %%%
\DIFdel{Acta Acustica united with Acustica}%DIFDELCMD < } {\bf %%%
\DIFdel{2017}%DIFDELCMD < }%%%
\DIFdel{, }%DIFDELCMD < {\em
%DIFDELCMD <   %%%
\DIFdel{103}%DIFDELCMD < }%%%
\DIFdel{,~430--443.
}%DIFDELCMD < 

%DIFDELCMD < \bibitem[Alsina-Pag{\`e}s \em{et~al.}(2016)Alsina-Pag{\`e}s, Hernandez-Jayo,
%DIFDELCMD <   Al{\'\i}as, and Angulo]{alsina2016design}
%DIFDELCMD < %%%
\DIFdelend %DIF >  ref 12 is the same as ref 16, please confirm. %R: Confirmed, deleted ref 12 and changed corresponding ref to 16
\DIFaddbegin \bibitem[Alsina-Pag{\`e}s et~al.(2016)Alsina-Pag{\`e}s, Hernandez-Jayo,
  Al{\'\i}as, and Angulo]{alsina2016design}
\DIFaddend Alsina-Pag{\`e}s, R.; Hernandez-Jayo, U.; Al{\'\i}as, F.; Angulo, I.
\newblock Design of a \DIFdelbegin \DIFdel{Mobile Low-Cost Sensor Network Using Urban Buses for
  Real-Time Ubiquitous Noise Monitoring}\DIFdelend \DIFaddbegin \DIFadd{mobile low-cost sensor network using urban buses for
  real-time ubiquitous noise monitoring}\DIFaddend .
\newblock {\em Sensors} {\bf 2016}, {\em 17},~57.

\bibitem[Alías and Socoró(2017)]{app7020146}
Alías, F.; Socoró, J.
\newblock Description of \DIFdelbegin \DIFdel{Anomalous Noise Events for Reliable Dynamic Traffic
  Noise Mapping in Real-Life Urban and Suburban Soundscapes}\DIFdelend \DIFaddbegin \DIFadd{anomalous noise events for reliable dynamic traffic
  noise mapping in real-life urban and suburban soundscapes}\DIFaddend .
\newblock {\em \DIFdelbegin \DIFdel{Applied Sciences}\DIFdelend \DIFaddbegin \DIFadd{Appl. Sci.}\DIFaddend } {\bf 2017}, {\em 7}\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{, 146.
}\DIFaddend 

\DIFdelbegin %DIFDELCMD < \bibitem[Gloaguen \em{et~al.}(2016)Gloaguen, Can, Lagrange, and
%DIFDELCMD <   Petiot]{gloaguen2016estimating}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibitem[Gloaguen et~al.(2016)Gloaguen, Can, Lagrange, and
  Petiot]{gloaguen2016estimating}
\DIFaddend Gloaguen, J.R.; Can, A.; Lagrange, M.; Petiot, J.F.
\newblock Estimating traffic noise levels using acoustic monitoring: \DIFdelbegin \DIFdel{a
  }\DIFdelend \DIFaddbegin \DIFadd{A~}\DIFaddend preliminary study.
\newblock  \DIFaddbegin \DIFadd{In Proceedings of the }\DIFaddend DCASE 2016, Detection and Classification of Acoustic Scenes and
  Events, \DIFdelbegin \DIFdel{2016, p.}\DIFdelend \DIFaddbegin {\DIFadd{Budapest, Hungary, 3  September}} \DIFadd{2016;}\DIFaddend ~4p.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < \bibitem[Aumond \em{et~al.}(2017)Aumond, Can, De~Coensel, Botteldooren,
%DIFDELCMD <   Ribeiro, and Lavandier]{aumond2017}
%DIFDELCMD < %%%
\DIFdelend %DIF > please confirm if the added information is right. %R: Confirmed
\DIFaddbegin \bibitem[Aumond et~al.(2017)Aumond, Can, De~Coensel, Botteldooren,
  Ribeiro, and Lavandier]{aumond2017}
{\DIFaddend Aumond, P.; Can, A.; De~Coensel, B.; Botteldooren, D.; Ribeiro, C.; Lavandier,
  C.\DIFaddbegin }
\DIFaddend \newblock Modeling \DIFdelbegin \DIFdel{Soundscape Pleasantness Using perceptual Assessments and
  Acoustic Measurements Along Paths in Urban Context}\DIFdelend \DIFaddbegin \DIFadd{soundscape pleasantness using perceptual assessments and
  acoustic measurements along paths in urban context}\DIFaddend .
\newblock {\em Acta\DIFdelbegin \DIFdel{Acustica united with Acustica}\DIFdelend \DIFaddbegin \DIFadd{~Acust. United Acust.}\DIFaddend } {\bf 2017}, {\em
  103},~430--443.

\DIFdelbegin %DIFDELCMD < \bibitem[Defr{\'e}ville \em{et~al.}(2006)Defr{\'e}ville, Pachet, Rosin, and
%DIFDELCMD <   Roy]{defreville2006automatic}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibitem[Defr{\'e}ville et~al.(2006)Defr{\'e}ville, Pachet, Rosin, and
  Roy]{defreville2006automatic}
\DIFaddend Defr{\'e}ville, B.; Pachet, F.; Rosin, C.; Roy, P.
\newblock Automatic recognition of urban sound sources.
\newblock  \DIFaddbegin \DIFadd{In Proceedings of the }\DIFaddend Audio Engineering Society Convention \DIFdelbegin \DIFdel{120. }\DIFdelend \DIFaddbegin \DIFadd{120, }{\DIFadd{Paris, France, 20--23}} \DIFadd{2006;   }\DIFaddend Audio Engineering Society\DIFdelbegin \DIFdel{,
   }\DIFdelend \DIFaddbegin \DIFadd{: }{\DIFadd{New York, NY, USA,}}  \DIFaddend 2006.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < \bibitem[Mydlarz \em{et~al.}(2015)Mydlarz, Shamoon, Baglione, and
%DIFDELCMD <   Pimpinella]{mydlarz2015design}
%DIFDELCMD < %%%
\DIFdelend %DIF > please confirm if the added information is right. %R: Location and dates changed from convention 121 to 120
\DIFaddbegin \bibitem[Mydlarz et~al.(2015)Mydlarz, Shamoon, Baglione, and
  Pimpinella]{mydlarz2015design}
\DIFaddend Mydlarz, C.; Shamoon, C.; Baglione, M.; Pimpinella, M.
\newblock The design and calibration of low cost urban acoustic sensing
  devices.
\newblock  \DIFdelbegin \DIFdel{EuroNoise }\DIFdelend \DIFaddbegin \DIFadd{In Proceedings of the EuroNoise 2015, }{\DIFadd{Maastricht, The Netherlands, 31 May--3 June}} \DIFaddend 2015.
\DIFdelbegin \DIFdel{Euronoise, 2015.
}%DIFDELCMD < 
...
\newblock {\em IEEE \DIFdelbegin \DIFdel{multimedia}\DIFdelend \DIFaddbegin \DIFadd{Multimedia}\DIFaddend } {\bf 1995}, {\em 2},~60--74.
...
\newblock The impact of sound quality on annoyance caused by road traffc noise:
  \DIFdelbegin \DIFdel{an }\DIFdelend \DIFaddbegin \DIFadd{An~}\DIFaddend influence of frequency spectra on annoyance.
\newblock {\em JSAE \DIFdelbegin \DIFdel{Review}\DIFdelend \DIFaddbegin \DIFadd{Rev.}\DIFaddend } {\bf 2000}, {\em 21},~225--230.

\bibitem[Anusuya and Katty(2009)]{anusuya2009}
Anusuya, M.; Katty, S.
\newblock Speech recognition by machine, a review.
\newblock {\em \DIFdelbegin \DIFdel{International Journal of Computer Science and Information
  Security}\DIFdelend \DIFaddbegin \DIFadd{Int. J. Comput. Sci. Inf. Secur.}\DIFaddend } {\bf 2009}, {\em 6},~181--205.

\DIFdelbegin %DIFDELCMD < \bibitem[Tzanztakis \em{et~al.}(2002)Tzanztakis, Essl, and
%DIFDELCMD <   Cook]{tzanetakis2002}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibitem[Tzanztakis et~al.(2002)Tzanztakis, Essl, and
  Cook]{tzanetakis2002}
\DIFaddend Tzanztakis, G.; Essl, G.; Cook, P.
\newblock Automatic musical genre classification of audio signals.
\newblock {\em IEEE \DIFdelbegin \DIFdel{Transactions on Speech  and Audio Processing}\DIFdelend \DIFaddbegin \DIFadd{Trans. Speech  Audio Process.}\DIFaddend } {\bf 2002},
  {\em 10},~293--302.
...
\newblock Universal background modeling for acoustic surveillance of urban
  traffic.
\newblock {\em \DIFdelbegin \DIFdel{Digital SignalProcessing}\DIFdelend \DIFaddbegin \DIFadd{Digit.~Signal~Process.}\DIFaddend } {\bf 2014}, {\em 31},~69--78.

\DIFdelbegin %DIFDELCMD < \bibitem[Aucouturier \em{et~al.}(2007)Aucouturier, Defreville, and
%DIFDELCMD <   Pachet]{aucouturier2007}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibitem[Aucouturier et~al.(2007)Aucouturier, Defreville, and
  Pachet]{aucouturier2007}
\DIFaddend Aucouturier, J.; Defreville, B.; Pachet, F.
\newblock The bag-of-frames approach to audio pattern recognition: A\DIFaddbegin \DIFadd{~}\DIFaddend sufficient
  model for urban soundscapes but not for polyphonic music.
\newblock {\em J. \DIFdelbegin \DIFdel{Ac}\DIFdelend \DIFaddbegin \DIFadd{Acoust}\DIFaddend . Soc. Am.} {\bf 2007}, {\em 122},~881--891.

\DIFdelbegin %DIFDELCMD < \bibitem[Foggia \em{et~al.}(2015)Foggia, Petkov, Saggese, Strisciuglio, and
%DIFDELCMD <   Vento]{foggia2015}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibitem[Foggia et~al.(2015)Foggia, Petkov, Saggese, Strisciuglio, and
  Vento]{foggia2015}
\DIFaddend Foggia, P.; Petkov, N.; Saggese, A.; Strisciuglio, N.; Vento, M.
\newblock Reliable detection of audio events in highly noisy environments.
\newblock {\em Pattern \DIFdelbegin \DIFdel{Recognition Letters}\DIFdelend \DIFaddbegin \DIFadd{Recognit. Lett.}\DIFaddend } {\bf 2015}, {\em 65},~22--28.

\bibitem[Kumar and Raj(2016)]{kumar2016}
Kumar, A.; Raj, B.
\newblock Features and Kernels for Audio Event Recognition. \DIFdelbegin %DIFDELCMD < \newblock \url{https://arxiv.org/abs/1607.05765}%%%
\DIFdel{,  }\DIFdelend 2016. 
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < \bibitem[Radhakrishnan \em{et~al.}(2005)Radhakrishnan, Divakaran, and
%DIFDELCMD <   Smaragdis]{radhakrishnan2005}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \newblock \DIFadd{Available online: \mbox{\url{https://arxiv.org/abs/1607.05765}} (}{\DIFadd{accessed on}}\DIFadd{). 
%DIF > please add accessed date.
}\bibitem[Radhakrishnan et~al.(2005)Radhakrishnan, Divakaran, and
  Smaragdis]{radhakrishnan2005}
\DIFaddend Radhakrishnan, R.; Divakaran, A.; Smaragdis, P.
\newblock Audio analysis for surveillance applications.
\newblock  \DIFaddbegin \DIFadd{In Proceedings of the }\DIFaddend IEEE Workshop on Applications of Signal Processing to Audio and
  Acoustics, \DIFaddbegin {\DIFadd{New Paltz, NY, USA, 16 October}} \DIFaddend 2005.
\DIFdelbegin %DIFDELCMD < 
...
\newblock {\em IEEE Signal \DIFdelbegin \DIFdel{Processing Letters}\DIFdelend \DIFaddbegin \DIFadd{Process. Lett.}\DIFaddend } {\bf 2017}, {\em 24},~279--283.

\bibitem[Piczak(2015)]{piczak2015}
Piczak, K.
\newblock Environmental sound classification with convolutional neural
  networks.
\newblock  \DIFaddbegin \DIFadd{In Proceedings of the }\DIFaddend IEEE 25th International Workshop on Machine Learning for Signal
  Processing, \DIFaddbegin {\DIFadd{Boston, MA, USA, 17--20 September }}\DIFaddend 2015.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < \bibitem[Khunarsal \em{et~al.}(2013)Khunarsal, Lursinsap, and
%DIFDELCMD <   Raicharoen]{khunarsal2013}
%DIFDELCMD < %%%
\DIFdelend %DIF > please confirm if the added information is right.
\DIFaddbegin \bibitem[Khunarsal et~al.(2013)Khunarsal, Lursinsap, and
  Raicharoen]{khunarsal2013}
\DIFaddend Khunarsal, P.; Lursinsap, C.; Raicharoen, T.
\newblock Very short time environmental sound classification based on
  spectrogram pattern matching.
\newblock {\em \DIFdelbegin \DIFdel{Information Sciences}\DIFdelend \DIFaddbegin \DIFadd{Inf. Sci.}\DIFaddend } {\bf 2013}, {\em 243},~57--74.
...
\newblock  \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{In Proceedings of the  }\DIFaddend 33rd International Congress and Exposition on Noise Control
  Engineering, \DIFaddbegin {\DIFadd{Prague, Czech Republic,  22--25 August }}\DIFaddend 2004.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < \bibitem[Cai \em{et~al.}(2006)Cai, Lu, Hanjalic, Zhang, and Cai]{cai2006}
%DIFDELCMD < %%%
\DIFdelend %DIF > please confirm if the added information is right. %R: Confirmed
\DIFaddbegin \bibitem[Cai et~al.(2006)Cai, Lu, Hanjalic, Zhang, and Cai]{cai2006}
\DIFaddend Cai, R.; Lu, L.; Hanjalic, A.; Zhang, H.; Cai, L.
\newblock A flexible framewok for key audio effects detection and auditory
  context inference.
\newblock {\em IEEE \DIFdelbegin \DIFdel{Transactions on Audio , Speech , and Language Processing}\DIFdelend \DIFaddbegin \DIFadd{Trans. Audio Speech Lang. Process.}\DIFaddend }
  {\bf 2006}, {\em 14},~1026--1039.

\DIFdelbegin %DIFDELCMD < \bibitem[Chu \em{et~al.}(2009)Chu, Narayanan, and Kuo]{chu2009}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibitem[Chu et~al.(2009)Chu, Narayanan, and Kuo]{chu2009}
\DIFaddend Chu, S.; Narayanan, S.; Kuo, C.
\newblock Environmental sound recognition with time-frequency audio features.
\newblock {\em IEEE\DIFdelbegin \DIFdel{Transactions on Audio , Speech , and Language Processing}\DIFdelend \DIFaddbegin \DIFadd{~Trans. Audio Speech Lang. Process.}\DIFaddend }
  {\bf 2009}, {\em 17},~1142--1158.

\DIFdelbegin %DIFDELCMD < \bibitem[Baugé \em{et~al.}(2013)Baugé, Lagrange, Andén, and
%DIFDELCMD <   Mallat]{bauge2013}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibitem[Baugé et~al.(2013)Baugé, Lagrange, Andén, and
  Mallat]{bauge2013}
\DIFaddend Baugé, C.; Lagrange, M.; Andén, J.; Mallat, S.
\newblock Representing environmental sounds using the separable scattering
  transform.
\newblock  \DIFaddbegin \DIFadd{In Proceedings of the }\DIFaddend 2013 IEEE International Conference on Acoustics, Speech and Signal
  Processing, \DIFaddbegin {\DIFadd{Vancouver, BC, Canada,  26--31 May }} \DIFaddend 2013.
\DIFdelbegin %DIFDELCMD < 
...
\newblock  \DIFaddbegin \DIFadd{In  Proceedings of the }\DIFaddend 23rd European Signal Processing Conference,  \DIFaddbegin {\DIFadd{Nice, France, 31 August--4 September }}\DIFaddend 2015.
\DIFdelbegin %DIFDELCMD < 
...
\newblock \DIFaddbegin \DIFadd{In Proceedings of the }\DIFaddend 2015 IEEE International Conference on Acoustics, Speech and Signal
  Processing, \DIFaddbegin {\DIFadd{Brisbane,  Australia, 19--24 April }} \DIFaddend 2015.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < \bibitem[Ntalampiras \em{et~al.}(2012)Ntalampiras, Potamitis, and
%DIFDELCMD <   Fakotakis]{ntalampiras2012}
%DIFDELCMD < %%%
\DIFdelend %DIF > please confirm if the added information is right. %R: Confirmed
\DIFaddbegin \bibitem[Ntalampiras et~al.(2012)Ntalampiras, Potamitis, and
  Fakotakis]{ntalampiras2012}
\DIFaddend Ntalampiras, S.; Potamitis, I.; Fakotakis, N.
\newblock Acoustic \DIFdelbegin \DIFdel{Detection of Human Activities in Natural Environments}\DIFdelend \DIFaddbegin \DIFadd{detection of human activities in natural environments}\DIFaddend .
\newblock {\em J. Audio Eng. Soc\DIFaddbegin \DIFadd{.}\DIFaddend } {\bf 2012}, {\em 60},~686--695.

\bibitem[Chachada and Kuo(2013)]{chachada2013}
Chachada, S.; Kuo, C.
\newblock Environmental sound recognition: A survey.
\newblock  \DIFaddbegin \DIFadd{In Proceedings of the }\DIFaddend 2013 Asia-Pacific Signal and Information Processing Association
  Annual Summit and Conference, \DIFaddbegin {\DIFadd{Kaohsiung, Taiwan, 29 October--1 November}} \DIFaddend 2013.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < \bibitem[Torija \em{et~al.}(2013)Torija, Ruiz, and Ramos-Ridao]{torija2013}
%DIFDELCMD < %%%
\DIFdelend %DIF > please confirm if the added information is right. %R: Confirmed
\DIFaddbegin \bibitem[Torija et~al.(2013)Torija, Ruiz, and Ramos-Ridao]{torija2013}
\DIFaddend Torija, A.; Ruiz, D.; Ramos-Ridao, A.
\newblock Application of a methodology for categorizing and differentiating
  ...
  attributes.
\newblock {\em J. \DIFdelbegin \DIFdel{Ac}\DIFdelend \DIFaddbegin \DIFadd{Acoust}\DIFaddend . Soc. Am.} {\bf 2013}, {\em 134},~791--802.
...
\newblock  \DIFaddbegin \DIFadd{In~Proceedings of the }\DIFaddend 1986 IEEE International Conference on Acoustics, Speech and Signal
  Processing, \DIFaddbegin {\DIFadd{Tokyo, Japan, 7--11 April }} \DIFaddend 1986.
\DIFdelbegin %DIFDELCMD < 
...
\newblock {\em J. \DIFdelbegin \DIFdel{Ac}\DIFdelend \DIFaddbegin \DIFadd{Acoust}\DIFaddend . Soc. Am.} {\bf 2010}, {\em 127},~\DIFdelbegin \DIFdel{884–895.
}\DIFdelend \DIFaddbegin \DIFadd{884--895.
}\DIFaddend 

\bibitem[cit(1993)]{citeulike:9580295}
\DIFaddbegin \emph{\DIFadd{ANSI S1.1-1986, (ASA 65-1986)---Specifications for Octave-Band and
  Fractional-Octave-Band Analog and Digital Filters}}\DIFadd{; }\DIFaddend {ANSI\DIFdelbegin \DIFdel{S1.1-1986, (ASA 65-1986)---Specifications for Octave-Band and
  Fractional-Octave-Band Analog and Digital Filters}%DIFDELCMD < }%%%
\DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{: Washington, DC, USA,}}  \DIFaddend 1993.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend %DIF > please confirm if the added information is right, and confirm if the layout  is right.
\bibitem[iec(2014)]{iec-norm}
\DIFaddbegin \emph{\DIFadd{IEC 61260-1:2014 ---Electroacoustics---Octave-Band and Fractional-Octave-Band
  Filters--Part 1: Specifications}}\DIFadd{; }\DIFaddend {IEC\DIFdelbegin \DIFdel{61260-1:2014 ---Electroacoustics - Octave-band and fractional-octave-band
  filters - Part 1: Specifications}%DIFDELCMD < }%%%
\DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{:~Geneva, Switzerland, }}\DIFaddend 2014.
\DIFdelbegin %DIFDELCMD < 
...
Couvreur, C.
\newblock Implementation of a \DIFdelbegin \DIFdel{one-third-octave filter bank }\DIFdelend \DIFaddbegin \DIFadd{One-Third-Octave Filter Bank }\DIFaddend in Matlab.
\newblock \DIFdelbegin %DIFDELCMD < \url{http://citeseer.ist.psu.edu/24150.html}%%%
\DIFdel{.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Available online: \mbox{\url{http://citeseer.ist.psu.edu/24150.html}} (}{\DIFadd{accessed on}} {\DIFadd{15 May 2017}}\DIFadd{).
%DIF > please add accessed date. %R: Added accessed date
}\DIFaddend \bibitem[Huffman(1952)]{huffman1952}
Huffman, D.
\newblock A method for the construction of minimum-redundancy codes.
\newblock {\em \DIFdelbegin \DIFdel{Proceedings of the }\DIFdelend \DIFaddbegin \DIFadd{Proc. }\DIFaddend IRE} {\bf 1952}, {\em 40},~1098--1101.

\DIFdelbegin %DIFDELCMD < \bibitem[Berzborn \em{et~al.}(2017)Berzborn, Bomhardt, Klein, Richter, and
%DIFDELCMD <   Vorländer]{itatoolbox2017}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibitem[Berzborn et~al.(2017)Berzborn, Bomhardt, Klein, Richter, and
  Vorländer]{itatoolbox2017}
\DIFaddend Berzborn, M.; Bomhardt, R.; Klein, J.; Richter, J.G.; Vorländer, M.
\newblock {T}he {ITA}-{T}oolbox: {A}n {\DIFdelbegin \DIFdel{O}\DIFdelend \DIFaddbegin \DIFadd{o}\DIFaddend }pen {\DIFdelbegin \DIFdel{S}\DIFdelend \DIFaddbegin \DIFadd{s}\DIFaddend }ource {MATLAB} {\DIFdelbegin \DIFdel{T}\DIFdelend \DIFaddbegin \DIFadd{t}\DIFaddend }oolbox for
  {\DIFdelbegin \DIFdel{A}\DIFdelend \DIFaddbegin \DIFadd{a}\DIFaddend }coustic {\DIFdelbegin \DIFdel{M}\DIFdelend \DIFaddbegin \DIFadd{m}\DIFaddend }easurements and {\DIFdelbegin \DIFdel{S}\DIFdelend \DIFaddbegin \DIFadd{s}\DIFaddend }ignal {\DIFdelbegin \DIFdel{P}\DIFdelend \DIFaddbegin \DIFadd{p}\DIFaddend }rocessing.
\newblock \DIFaddbegin \DIFadd{In Proceedings of the }\DIFaddend 43th Annual German Congress on Acoustics, Kiel\DIFdelbegin \DIFdel{(Germany), 6 Mar 2017
  - 9 Mar 2017}\DIFdelend \DIFaddbegin \DIFadd{, Germany}\DIFaddend , \DIFaddbegin \DIFadd{6--9 March }\DIFaddend 2017.

\DIFdelbegin %DIFDELCMD < \bibitem[Salamon \em{et~al.}(2014)Salamon, Jacoby, and Bello]{salamon2014}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibitem[Salamon et~al.(2014)Salamon, Jacoby, and Bello]{salamon2014}
\DIFaddend Salamon, J.; Jacoby, C.; Bello, J.
\newblock A \DIFdelbegin \DIFdel{Dataset and Taxonomy for Urban Sound Research}\DIFdelend \DIFaddbegin \DIFadd{dataset and taxonomy for urban sound research}\DIFaddend .
\newblock \DIFaddbegin \DIFadd{In Proceedings of the }\DIFaddend 22nd ACM international conference on Multimedia, \DIFaddbegin {\DIFadd{Orlando, FL, USA, 3--7 November }}\DIFaddend 2014.
%DIF > please confirm if the added information is right. %R: Confirmed

\DIFaddbegin \bibitem[Ntalampiras(2013)]{ntalampiras2013}
\DIFadd{Ntalampiras, S.
}\newblock {\DIFadd{A novel holistic modeling acoustic approach for generalized sound
  recognition.}}
\newblock {\em \DIFadd{IEEE Signal Process. Lett.}} {\bf \DIFadd{2013}}\DIFadd{, }{\em \DIFadd{20}}\DIFadd{,~185--188.
%DIF > please confirm if ref 48 is the same as ref 55. %R: Title of ref 55 is now correct
}\DIFaddend \bibitem[Ellis(2005)]{ellis2005}
Ellis, D.
\newblock {PLP} and {RASTA} (and {MFCC}, and \DIFdelbegin \DIFdel{inversion}\DIFdelend \DIFaddbegin \DIFadd{Inversion}\DIFaddend ) in {M}atlab. \DIFdelbegin %DIFDELCMD < \newblock \url{http://www.ee.columbia.edu/~dpwe/resources/matlab/rastamat/}%%%
\DIFdel{,
  }\DIFdelend 2005.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < \bibitem[Ntalampiras(2013)]{ntalampiras2013}
%DIFDELCMD < %%%
\DIFdel{Ntalampiras, S.
}\DIFdelend \newblock \DIFdelbegin \DIFdel{A Novel Holistic Modeling Acoustic Approach for Generalized Sound
  Recognition.
}%DIFDELCMD < \newblock %%%
\DIFdelend \DIFaddbegin \DIFadd{Available online: }\url{http://www.ee.columbia.edu/~dpwe/resources/matlab/rastamat/} \DIFadd{(}\DIFaddend {\DIFdelbegin %DIFDELCMD < \em %%%
\DIFdel{IEEE Signal Processing Letters}\DIFdelend \DIFaddbegin \DIFadd{accessed on}\DIFaddend } {\DIFdelbegin %DIFDELCMD < \bf %%%
\DIFdel{2013}\DIFdelend \DIFaddbegin \DIFadd{6 March 2017}\DIFaddend }\DIFdelbegin \DIFdel{, }%DIFDELCMD < {\em %%%
\DIFdel{20}%DIFDELCMD < }%%%
\DIFdel{,~185--188.
 }%DIFDELCMD < 
...
\DIFdelend \DIFaddbegin \DIFadd{).
 %DIF > please add accessed date. %R: Added accessed date
}\DIFaddend \bibitem[Baldwin and French(1990)]{baldwin1990forensic}
Baldwin, J.; French, P.
\newblock {\em Forensic \DIFdelbegin \DIFdel{phonetics}\DIFdelend \DIFaddbegin \DIFadd{Phonetics}\DIFaddend }; Pinter Publishers\DIFaddbegin \DIFadd{: }{\DIFadd{London, }}{\DIFadd{UK}}\DIFaddend , 1990.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend %DIF > please add city. %R: Added city
\bibitem[Bo{\"e}(2000)]{boe2000forensic}
Bo{\"e}, L.J.
\newblock Forensic voice identification in France.
\newblock {\em Speech \DIFdelbegin \DIFdel{Communication}\DIFdelend \DIFaddbegin \DIFadd{Commun.}\DIFaddend } {\bf 2000}, {\em 31},~205--224.

\bibitem[Kuwabara(1996)]{kuwabara1996acoustic}
Kuwabara, H.
\newblock Acoustic properties of phonemes in continuous speech for different
  speaking rate.
\newblock  \DIFdelbegin \DIFdel{Spoken Language, 1996. ICSLP 96. Proceedings ., }\DIFdelend \DIFaddbegin \DIFadd{In~Proceedings of the }\DIFaddend Fourth International  Conference on \DIFdelbegin \DIFdel{. IEEE,  1996, Vol.}\DIFdelend \DIFaddbegin \DIFadd{Spoken Language (ICSLP 96),  }{\DIFadd{Philadelphia, PA, USA, 3--6 October}} \DIFadd{1996; Volume}\DIFaddend ~4, pp. 2435--2438.
\DIFdelbegin %DIFDELCMD < 
...
Rosen, S.
\newblock Temporal information in speech: \DIFdelbegin \DIFdel{acoustic}\DIFdelend \DIFaddbegin \DIFadd{Acoustic}\DIFaddend , auditory and linguistic
  aspects.
\newblock {\em \DIFdelbegin \DIFdel{Philosophical Transactions of the Royal Society of London B :
  Biological Sciences}\DIFdelend \DIFaddbegin \DIFadd{Philos. Trans. R. Soc. Lond. B Biol. Sci.}\DIFaddend } {\bf 1992}, {\em 336},~367--373.

\bibitem[itu(2003)]{itup835}
{\DIFaddbegin \em \DIFaddend ITU-T P.835: Subjective \DIFdelbegin \DIFdel{test methodology for evaluating speech communication
  systems that include noise suppression algorithm}\DIFdelend \DIFaddbegin \DIFadd{Test Methodology for Evaluating Speech Communication
  Systems That Include Noise Suppression Algorithm}\DIFaddend }\DIFdelbegin \DIFdel{,  }\DIFdelend \DIFaddbegin \DIFadd{; }{\DIFadd{ITU: Geneva, Switzerland,}} \DIFaddend 2003.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < \bibitem[Ntalampiras \em{et~al.}(2008)Ntalampiras, Ganchev, and
%DIFDELCMD <   Potamitis]{ntalampiras2008}
%DIFDELCMD < %%%
\DIFdelend %DIF > please confirm if the added information is right, and confirm if the layout  is right.
\DIFaddbegin \bibitem[Ntalampiras et~al.(2008)Ntalampiras, Ganchev, and
  Potamitis]{ntalampiras2008}
\DIFaddend Ntalampiras, S.; Ganchev, T.; Potamitis, I.
\newblock \DIFdelbegin \DIFdel{A Novel Holistic Modeling Acoustic Approach for Generalized Sound
  Recognition.}\DIFdelend \DIFaddbegin {\DIFadd{Objective comparison of speech enhancement algorithms under real world conditions.}}
\DIFaddend \newblock  \DIFaddbegin \DIFadd{In Proceedings of the }\DIFaddend 1st \DIFdelbegin \DIFdel{international conference }\DIFdelend \DIFaddbegin \DIFadd{International Conference }\DIFaddend on PErvasive Technologies Related to
  Assistive Environments, \DIFaddbegin {\DIFadd{Athens, Greece, 15--19 July}} \DIFaddend 2008.
\DIFdelbegin %DIFDELCMD < 
...
Griffin, D.; Lim, J.
\newblock Signal \DIFdelbegin \DIFdel{Estimation from Modified Short-Time Fourier Transform}\DIFdelend \DIFaddbegin \DIFadd{estimation from modified short-time fourier transform}\DIFaddend .
\newblock {\em IEEE \DIFdelbegin \DIFdel{Transactions on Acoustics, Speech , and Signal Processing}\DIFdelend \DIFaddbegin \DIFadd{Trans. Acoust. Speech Signal Process.}\DIFaddend }
  {\bf 1984}, {\em 32},~236--243.

\bibitem[iec(2013)]{iec-norm2}
{\DIFaddbegin \em \DIFaddend IEC 61672-1:\DIFdelbegin \DIFdel{2013 ---Electroacoustics - Sound level meters - Part }\DIFdelend \DIFaddbegin \DIFadd{2013---Electroacoustics---Sound Level Meters---Part }\DIFaddend 1:
  Specifications}\DIFdelbegin \DIFdel{,  }\DIFdelend \DIFaddbegin \DIFadd{; }{\DIFadd{IEC: Geneva, Switzerland,}}\DIFadd{~}\DIFaddend 2013.
\DIFdelbegin %DIFDELCMD < 
