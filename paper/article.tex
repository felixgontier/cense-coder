...
To evaluate the impact of the frame rate on intelligibility two objective metrics are considered: the Coherence Speech Intelligibility Index\cite{kates2005} (CSII) and frequency-weighted segmental SNR\cite{hu2008} (fwSNRseg) by comparing original unaltered samples and recovered audio. These two indicators have been shown to correlate well with perceptual tests\cite{ma2009}, although results are only available for lesser degradations and distortions than the ones we consider in this study. For this reason, a small-scale perceptual test is also conducted. We use a dataset of clean speech recordings comprised of 9 french sentences enunciated by 3 male and 3 female speakers. The recordings are encoded with analysis frame rates of 50~Hz, 16~Hz, 8~Hz and 4~Hz for a total of 216 audio excerpts. At each test iteration, a listener is presented with sentences and asked both to transcribe the utterances and rate the global intelligibility between 1 and 5. Each sentence is tested twice in order to palliate the reduced number of participants. The listening bias induced is attenuated by using the 9 sentences only once for the two lower and higher frame rates. Furthermore, the global order of examples as well as speakers are randomized. Nine subjects of ages ranging from 17 to 60 and with normal hearing capacity participated.\\
...
Finally, objective intelligibility metrics are estimated for the studied parameters and compared to an informal perceptual evaluation \footnote{Decoded speech utterances for various conditions can be listened to at \url{http://}}. Figures~\ref{fig:csii_tob_fps} and \ref{fig:fwsnrseg_tob_fps} shows both the CSII and fwSNRseg indicators for varying frames per second in analysis. Figure~\ref{fig:subj_int} presents the results of the preliminar perceptual test for the same setup. We acknowledge that it is surely biased by the reduced number of participants and scarcity of tested audio material. However, it is sufficient to judge the relevance of objective indicators in our conditions and study the intelligibility of fast acoustical measurements.\\

Perceptual test results are in accordance with the formulated concerns. First, phonems separation is mandatory in order to ensure intelligibility. This implies an analysis frame rate significantly larger than that of phonems in the considered language. Here we observe a limit of about 10~Hz corresponding to a duration of 100~ms, rendering the studied 8~Hz measurements inintelligible. Few words were understood correctly as a result of coincidentally adequate frame timings, ie. analysis frames matching phonem utterances for a short duration. Similarly, framing effects induced by the proposed processing scheme yielded errors for higher fps transcriptions and a globally lower comprehension as perceived by the subjects.\\

The CSII accounts for the spectrogram distance between clean and distorted audio at each point in time and frequency. This includes the phase, which we discard early in the representation step. The error induced by forward and inverse third-octave bands transformation as well as operations reducing data dimensionality further contribute to very high point-wise disparities. In fact, the signal is too distorted for the proposed SNR normalization to ensure CSII values in the 0-1 range. When compared to perceptual test results, the CSII shows no clear separation in the 10 frames per second neighbourhood. It is therefore not a good indicator in our conditions.\\
...
\begin{figure}[htbp]
	...
	\caption{Results of the perceptual intelligibility test with subjective normalized for visibility. A limit for intelligibility appears at around 10~Hz, corresponding to the average duration of a phonem.}
	\label{fig:subj_int}
\end{figure}
\section{Conclusion}
