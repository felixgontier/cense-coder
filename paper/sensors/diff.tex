...
In the case of \DIFaddbegin \DIFadd{urban }\DIFaddend acoustic monitoring, the data of interest \DIFdelbegin \DIFdel{can be quantitative such as standard acoustic indicators (LAeq, ...) or qualitative }\DIFdelend \DIFaddbegin \DIFadd{typically rely on standardized high-quality measurements of energetic indicators }\DIFaddend such as the \DIFdelbegin \DIFdel{presence of sound sources of interest (bird calls, sirens, ...). The latter allows us }\DIFdelend \DIFaddbegin \DIFadd{equivalent sound level $L_{eq}$ in dB SPL or its A-weighted counterpart $L_{Aeq}$ in dBA. However, while these indicators have proven to correlate well with perceptual evaluations for negative impact sound environments \mbox{%DIFAUXCMD
...
\section{\DIFdelbegin \DIFdel{Background}\DIFdelend \DIFaddbegin \DIFadd{Encoding scheme}\DIFaddend } \DIFdelbegin %DIFDELCMD < \label{sec:background}
%DIFDELCMD < %%%
