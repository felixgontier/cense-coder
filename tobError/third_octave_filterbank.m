function G = third_octave_filterbank(sr, N, bands, l)
...
for ind_wm = 1:length(wm)
    ...
    if ind_wm == 1 % No ind_wm-1 this time
        ...
        P = round(N*(w1(ind_wm)-wm(ind_wm-1))/ws)-1; % Right
    end
    if P < 1
...
P = round(N*(w1(end)*wd*wd-wm(end))/ws)-1;
p = -P:P;
phi = .5*(p./P+1);
for ind_l = 1:l
    phi = sin(pi/2*phi).^2;
...
k_i = round(N*w1*wd*wd/ws)+1;
a = cos(pi/2*phi);
G(length(wm), k_i(end)+p(1):min(k_i(end)+p(end), end)) = a(1:size(G(length(wm), k_i(end)+p(1):min(k_i(end)+p(end), end)), 2));
